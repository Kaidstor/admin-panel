...
function formValidate<T extends z.ZodRawShape>(
  formData: FormData,
  schema: ZodObject<T>
) {
  ...
  const fieldErrors = form.error.flatten().fieldErrors as {
    [P in keyof T]?: string[];
  };

  errors = Object.keys(fieldErrors).reduce(
    (acc, el) => ({
      [el]: fieldErrors[el]![0],
      ...acc,
    }),
...
export async function superValidate<T extends z.ZodRawShape>(
  event: ServerLoadEvent | RequestEvent,
  schema: ZodObject<T>
): Promise<ValidationResult<T>> {
  const constraints = Object.keys(schema.shape).reduce(
    (acc, field) => ({
      ...acc,
...
function createZodSchema(constraints: SchemaConstraints) {
  ...
  Object.entries(constraints).forEach(([fieldName, fieldConstraints]) => {
    ...
    if (fieldConstraints) {
      if (
        ["min", "max", "email"].some((el) =>
          fieldConstraints.map((el) => el.kind).includes(el)
        )
      )
        fieldSchema = z.string();
      else fieldSchema = z.number();

      if (fieldSchema instanceof z.ZodString) {
        fieldSchema = applyChecksToStringFieldSchema(
          fieldSchema,
          fieldConstraints
        );
      }
      if (fieldSchema instanceof z.ZodNumber) {
        fieldSchema = applyChecksToNumberFieldSchema(
          fieldSchema,
          fieldConstraints
        );
      }
...
function applyChecksToStringFieldSchema(
...
function applyChecksToNumberFieldSchema(
