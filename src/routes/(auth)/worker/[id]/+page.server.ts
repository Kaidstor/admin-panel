...
import { stringWithoutDigits } from "$lib/utils";
...
export const actions = {
   updateWorkerProfile: async (event) => {
      ...
      const [user] = await db.select().from(db_user_heads).where(eq(db_user_heads.id, +params.id))
      ...
      const updated_user = await db.update(db_users).set({ name }).where(eq(db_users.id, +params.id)).returning()
...
const updateWorkerSchema = z.object({
   name: z.string().min(2, { message: 'Минимальная длина имени 2 символа' }).max(20, { message: 'Максимальная длина имени 20 символов' }).refine(stringWithoutDigits, { message: "Строка не должна содержать цифр" }),
})