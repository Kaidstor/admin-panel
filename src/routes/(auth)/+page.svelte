...
<div class="flex justify-between">
  ...
  <Button size="md" class="flex gap-3 items-center" onclick={() => formCreateVenue?.showModal()}>
    <Plus />
  </Button>
</div>
  ...
  {#each data.venues as item}
    ...
    >
      <div class="flex justify-between">
        ...
        <Button class="py-1 text-md" type="link" href={`/tariff`}>
          <Crown />
          <p class="font-sofia">+ PRO</p>
        </Button>
      </div>
      <div class="flex gap-4 space-between">
        <div class="flex flex-col gap-1 mt-6 flex-1">
          <p class="text-lg">{item.meta.address}</p>
          <p>
            <span class="text-primary">13 дней</span> до отключения тарифа
          </p>
        </div>
...
<dialog bind:this={formCreateVenue} class="px-12 py-8 rounded-lg bg-dark">
  ...
  <p class="text-4xl text-white font-bold">Добавить заведение</p>
  <FormAddVenue
     class="mt-8"
     action="?/addVenue"
